...
> ⚠️ **2026-09-22 修订 · 移动云开机能力已移除**：移动云「SC/ZTE 自适应开机」及其底层直连开机引擎（`app/ydpc/boot_engine.js`）已按用户要求**整体删除**。移动云侧现在只保留账号登录、SOHO 心跳、ZTEC CAG 握手、官方 MQTT 长连接与状态监控，以及官方 SOHO 的**关机 / 重启**——**不再具备任何开机 / 唤醒能力**。如需开机，请在移动云官方 App 中连接一次。天翼云不受影响。
...
- **移动云电脑（YDPc）**：SOHO REST API（HMAC-SHA256 签名 + RSA 分段加密）+ ZTEC CAG TCP 三阶段握手保活 + 官方 MQTT 3.1.1 over TLS 长连接，**100% 原生 Node.js 移植，零 Python 依赖**；
...
- **[ydpc-keeplive](https://github.com/zhaoboy9692/ydpc-keeplive)** —— 中国移动云电脑保活脚本，本项目的移动云 SOHO 签名加密与 CAG 握手逻辑参考自该项目，并以原生 Node.js 重新实现（注：该项目的 SC/ZTE 开机引擎曾一度被移植进来，已于 2026-09-22 按用户要求整体移除）；
